...
async def get_token(bot, userid, link, message, file_id):
    user = await bot.get_users(userid)
    try:
        kk, file_id = message.command[1].split("_", 1)
        pre = 'checksubp' if kk == 'filep' else 'file'
        file_id = f"{pre}#{file_id}"
    except IndexError:
        pass
    ...
    link = f"{link}/verify-{user.id}-{token}-{file_id}"
    shortened_verify_url = await get_verify_shortened_link(link)
