...
async def get_rules(Client, m: Message):
    db = Rules(m.chat.id)
...
async def priv_rules(Client, m: Message):
    db = Rules(m.chat.id)
    if m and not m.from_user:
        return
...
async def clear_rules(Client, m: Message):
    db = Rules(m.chat.id)
    if m and not m.from_user:
        return
...
async def clearrules_callback(Client, q: CallbackQuery):
